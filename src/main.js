...
/*
const orderedList = document.querySelector(".orderArray");
orderArray.map(option => {
  option.addEventListener("click", () => {
    const term = option.getAttribute("data-value");
    orderedList.innerHTML = "";
    const orderedArray =  orderList(championList, term);
...
const playButtonContainer1 = document.getElementById("buttonContainer");
...
const modalStatsUncle = champion => {
  ...
  champion.tags.map((tag, index) => {
    ...
    modalRolesP.innerHTML =
      champion.tags.length - 1 !== index ? `${tag},` : `${tag}`;
